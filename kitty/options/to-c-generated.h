...
static void
convert_from_python_cursor_unfocused_opacity(PyObject *val, Options *opts) {
    opts->cursor_unfocused_opacity = PyFloat_AsFloat(val);
}

static void
convert_from_opts_cursor_unfocused_opacity(PyObject *py_opts, Options *opts) {
    PyObject *ret = PyObject_GetAttrString(py_opts, "cursor_unfocused_opacity");
    if (ret == NULL) return;
    convert_from_python_cursor_unfocused_opacity(ret, opts);
    Py_DECREF(ret);
}
...
static void
convert_from_python_claude_context_breakdown(PyObject *val, Options *opts) {
    opts->claude_context_breakdown = PyObject_IsTrue(val);
}

static void
convert_from_opts_claude_context_breakdown(PyObject *py_opts, Options *opts) {
    PyObject *ret = PyObject_GetAttrString(py_opts, "claude_context_breakdown");
    if (ret == NULL) return;
    convert_from_python_claude_context_breakdown(ret, opts);
    Py_DECREF(ret);
}
...
convert_opts_from_python_opts(PyObject *py_opts, Options *opts) {
    ...
    if (PyErr_Occurred()) return false;
    convert_from_opts_cursor_unfocused_opacity(py_opts, opts);
    if (PyErr_Occurred()) return false;
    ...
    if (PyErr_Occurred()) return false;
    convert_from_opts_claude_context_breakdown(py_opts, opts);
    if (PyErr_Occurred()) return false;
