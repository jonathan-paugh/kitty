...
:code:`self`, :code:`overlay_parent`, :code:`alternate_screen`.  Active windows are the windows that are
...
window, when the self window is an overlay. The value
:code:`alternate_screen` matches windows running a full-screen application,
which is useful with :code:`--when-focus-on` to turn off mappings that only
make sense against the scrollback.
